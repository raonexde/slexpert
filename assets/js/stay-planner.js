(function () {
    ...
    function renderHotels() {
        ...
        rule.textContent = selectedHotel ? data.labels.pricingRule
            .replace('%g', String(selectedHotel.standard_guests))
            .replace('%e', String(selectedHotel.extra_bed_percent))
            .replace('%a', String(selectedHotel.child_max_age))
            .replace('%c', String(selectedHotel.child_percent)) : '';
    }
    function normaliseRooms() {
        var selectedHotel = hotel();
        var occupancy = people();
        var maximum = selectedHotel ? Math.max(Number(selectedHotel.standard_guests || 2), Number(selectedHotel.maximum_guests || 3)) : 3;
    ...
    function renderServices() {
        ...
        var services = data.services.filter(function (service) {
            return Number(service.destination_id) === Number(selectedHotel.destination_id) && service.type === activeServiceType;
        });
        if (!services.length) {
    ...
    function calculate() {
        ...
        var standardGuests = Math.max(1, Number(selectedHotel.standard_guests || 2));
        var maximumGuests = Math.max(standardGuests, Number(selectedHotel.maximum_guests || 3));
        ...
        var extraFactor = Math.max(0, Number(selectedHotel.extra_bed_percent || 0)) / 100;
        var childFactor = Math.max(0, Number(selectedHotel.child_percent || 0)) / 100;
        var roomCost = Number(selectedHotel.rate) * occupancy.rooms * occupancy.nights;
        var extraCost = (Number(selectedHotel.rate) * extraFactor * extraAdults * occupancy.nights) +
            (Number(selectedHotel.rate) * extraFactor * childFactor * extraChildren * occupancy.nights);
        ...
        return {
            ...
            maximumCapacity: maximumCapacity, occupancy: occupancy, plan: plan
        };
    }
    function renderSummary() {
        ...
        selectedStay.innerHTML = '<small>' + esc(text(selectedHotel, 'destination_name')) + '</small><strong>' + esc(text(selectedHotel, 'name')) + '</strong><span>' +
            esc(result.occupancy.rooms + ' ' + data.labels.rooms + ' · ' + nightText(result.occupancy.nights)) + '</span>' +
    ...
    nightsInput.addEventListener('change', renderSummary);
