...
if ($id) {
    ...
    if ($hotelMode && $item['type'] !== 'accommodation') { flash('error','Dieser Reisebaustein ist kein Hotel.'); redirect('admin/hotels.php'); }
}
$mealPlans = [];
if ($id) {
...
$adminPage=$hotelMode?'hotels':'catalog';$adminTitle=$hotelMode?($id?'Hotel bearbeiten':'Hotel erstellen'):($id?'Baustein bearbeiten':'Baustein erstellen');require __DIR__.'/_header.php';
?>
<div class="admin-content narrow">
    <a class="back-admin" href="<?= $hotelMode?'hotels.php':'catalog.php' ?>">← <?= $hotelMode?'Hotelverwaltung':'Reisebausteine' ?></a>
    <div class="page-heading compact"><div><p><?= $hotelMode?'UNTERKUNFT':'ANGEBOT' ?></p><h1><?= $hotelMode?($id?'Hotel bearbeiten':'Neues Hotel'):($id?'Reisebaustein bearbeiten':'Neuer Reisebaustein') ?></h1><span><?= $hotelMode?'Stammdaten, Klassifizierung, Bilder, Verpflegung und Lieferantendaten. Zimmerkategorien werden nach dem Speichern separat verwaltet.':'Unterkunft, Sehenswürdigkeit, Aktivität, Restaurant, Gewürzgarten, Shop oder Zusatzleistung.' ?></span></div><?php if($hotelMode&&$id):?><a class="primary-button" href="hotel-rooms.php?hotel_id=<?= $id ?>">Zimmer & Leistungen →</a><?php endif;?></div>
    <form class="editor-form" method="post" enctype="multipart/form-data">
        ...
        <section class="admin-card form-card" data-occupancy-section><div class="card-head"><div><h2>Fallback-Belegung & Kinder</h2><p>Diese Werte gelten nur, wenn noch keine eigene Zimmerkategorie vorhanden ist.</p></div></div><div class="form-grid thirds">
