...
<main class="itinerary-sheet">
    ...
    <section class="journey-facts stay-facts">
        ...
        <div><small><?= e(t('Zimmer','Rooms')) ?></small><strong><?= (int)$request['rooms'] ?></strong></div>
    </section>
    <section class="stay-hotel-print"><div><small><?= e(t('GEWÄHLTES HOTEL','SELECTED HOTEL')) ?> · <?= e(lang()==='en'?($destination['name_en']??''):($destination['name_de']??'')) ?></small><h2><?= e(lang()==='en'?$request['hotel_name_en']:$request['hotel_name_de']) ?></h2><?php if($hotel): $roomTypeName=lang()==='en'?$hotel['room_type_name_en']:$hotel['room_type_name_de'];?><p><?= e(lang()==='en'?$hotel['meta_en']:$hotel['meta_de']) ?></p><div class="stay-hotel-meta"><span><?= (int)$request['rooms'] ?> <?= e(t('Zimmer','room(s)')) ?><?= $roomTypeName?' · '.e($roomTypeName):'' ?></span><span><?= e(money_precise($hotel['unit_price'])) ?> / <?= e(t('Zimmer / Nacht','room / night')) ?></span><?php if($mealPlanName):?><span><?= e(t('Verpflegung','Meal plan')) ?>: <?= e($mealPlanName) ?></span><?php endif;?></div><?php endif;?></div><strong><?= e(money_precise((float)$request['room_cost']+(float)$request['extra_bed_cost']+(float)$request['meal_cost'])) ?></strong></section>
